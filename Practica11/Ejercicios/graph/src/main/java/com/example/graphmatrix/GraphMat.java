...
public class GraphMat<E> {
    ...
    private void addEdges(int index, boolean[] visited, PriorityQueue<Edge<E>> queue) {
        ...
        for (int i = 0; i < size; i++) {
            if (adjMatrix[index][i] && !visited[i]) {
                queue.add(new Edge<>(new Vertex<>(verticesList.get(index), index), new Vertex<>(verticesList.get(i), i),
                        weightMatrix[index][i]));
            }
        }
    }
    ...
    public List<Edge<E>> kruskal() {
        ...
        for (int i = 0; i < size; i++) {
            for (int j = 0; j < size; j++) {
                if (adjMatrix[i][j]) {
                    edges.add(new Edge<>(new Vertex<>(verticesList.get(i), i), new Vertex<>(verticesList.get(j), j),
                            weightMatrix[i][j]));
                }
            }
        }
