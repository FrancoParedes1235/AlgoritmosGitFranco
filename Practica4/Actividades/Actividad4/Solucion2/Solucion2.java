public class Solucion2 {
    ...
    public static int moda2(int array[]) {
        ...
        while (first <= end) {
            // Si el elemento actual es igual al elemento en la posición 'first', se incrementa la frecuencia
            if (array[p] == array[first]) {
                frec++; // Incrementa la frecuencia
                first++; // Mueve el índice al siguiente elemento
            } else {
                // Si la frecuencia actual es mayor que la frecuencia máxima encontrada hasta ahora,
